...
import { useCooldown } from "./hook/Cooldown";
...
const LoginModal = ({ isOpen, onClose, onComplete }) => {
  ...
  const { locked, timeLeft } = useCooldown(20); // For the coolDown button time 
  ...
  const handleOtpSubmit = () => {
    ...
    const res = fetch("http://localhost:5001/auth",{
      method: "POST",
      headers: {
        ...
        <div className="relative bg-[#0F1014] rounded-[22px] p-8 h-[550px] flex flex-col">
          ...
          <div className="flex gap-2 mb-8 justify-center">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className={`h-1 rounded-full transition-all duration-500 ${step >= i ? "w-8 bg-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.5)]" : "w-2 bg-zinc-800"}`}
              />
            ))}
          </div>
            ...
            {step === 1 && (
              ...
              >
                ...
                <p className="text-zinc-500 text-sm mb-8 px-4">
                  Confirm your identity via National ID
                  (Aadhar).
                </p>
                ...
                <div className="w-full space-y-4">
                  <input
                    ...
                    value={formatAadhar(aadhar)}
                     onChange={(e) => {const digitsOnly = e.target.value.replace(/\D/g, "");setAadhar(digitsOnly);}}
            ...
            {step === 2 && (
              ...
              >
                ...
                <p className="text-zinc-500 text-sm mb-8">
                  Enter the 6-digit encryption key sent to your device.
                </p>
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="flex-1 bg-black/40 rounded-xl border border-zinc-800 p-4 mb-6 overflow-y-auto font-mono text-xs text-zinc-400 leading-relaxed custom-scrollbar">
                  <p className="mb-2 text-zinc-300 font-bold">
                    1. PROTOCOL USAGE
                  </p>
                  <p className="mb-4">
                  ...
                  <p className="mb-4">
                    Your personal data (Aadhar) remains off-chain. Only
                    Zero-Knowledge proofs are generated and stored.
                  </p>
                  <p className="mb-2 text-zinc-300 font-bold">3. LIABILITY</p>
                  <p>
                    User accepts full responsibility for private key management
                    and transaction signing.
                  </p>
                </div>
                <label
                  className={`flex items-start gap-4 mb-6 p-4 rounded-xl border transition-all duration-300${locked? "opacity-50 pointer-events-none": "cursor-pointer hover:bg-white/5 border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.15)]"}`}>
                  <div
                    className={`mt-1 w-5 h-5 rounded border flex items-center justify-center transition-all duration-300${agreed? "bg-purple-500 border-purple-500": 
                        locked ? "border-zinc-600 bg-black" : "border-purple-400 bg-purple-500/10 shadow-[0_0_10px_rgba(168,85,247,0.35)]"}`}>
                    {agreed && (
                  ...
                  <span
                    className={`text-sm select-none transition-colors duration-300 ${locked ? "text-zinc-400" : "text-zinc-200"}`}>
            ...
            {step === 4 && (
              ...
              >
                ...
                <p className="text-zinc-400 mb-8 max-w-xs">
                  Access granted. Connect your wallet to finalize the handshake.
                </p>
